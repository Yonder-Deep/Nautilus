...
class DiveController:
    ...
    def start_dive(self, to_depth=0, dive_length=0):
        ...
        def modulo(x, y):
            return x % y if x > 0 else -1 * (abs(x) % y)
        ...
        def turn_error(target, heading):
            return modulo(target - heading, 360)
        ...
        while time.time() < start_time + 60:
            ...
            heading_correction = self.pid_heading.pid(
                turn_error(start_heading, heading)
            )
            ...
            print(
                "Depth_Correction: {}\tPitch_Correction: {}\tHeading_Correction: {}\n".format(
                    depth_correction, pitch_correction, heading_correction
                )
            )
            ...
            try:
                self.mc.update_motor_speeds(
                    [0, side_motor_value, front_motor_value, back_motor_value]
                )
            except:
            ...
            if target_met and time.time() > target_met_time + dive_length:
                print(
                    "Maintained depth for {} seconds, ending dive".format(dive_length)
                )
                break

            print(
                "Currently diving. Current depth: {}, Target depth: {}".format(
                    depth, to_depth
                )
            )
            time.sleep(0.1)
    ...
    def update_pitch_pid(self):
        ...
        print(
            "Updating pitch PID constants: {} {} {}".format(
                constants.P_PITCH, constants.I_PITCH, constants.D_PITCH
            )
        )
    ...
    def update_depth_pid(self):
        ...
        print(
            "Updating depth PID constants: {} {} {}".format(
                constants.P_DEPTH, constants.I_DEPTH, constants.D_DEPTH
            )
        )
    ...
    def update_heading_pid(self):
        ...
        print(
            "Updating heading PID constants: {} {} {}".format(
                constants.P_HEADING, constants.I_HEADING, constants.D_HEADING
            )
        )
